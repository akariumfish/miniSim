...
public class nNumPanel extends nWindowPanel {
	  ...
	  void build_ui(sValue v) {
	    ...
	    lim_min_v = getDrawer(0,3).addModel("Button-S1-P2", "l min").setSwitch().setSwitchState(val.limited_min)
	    		.addEventSwitchOn(new nRunnable() { public void run() { 
	    			if(event_paused) val.pauseEvent();
	    			val.set_limit_min(true); } }).setInfo("limited min")
	    		.addEventSwitchOff(new nRunnable() { public void run() { 
	    			if(event_paused) val.pauseEvent();
			    val.set_limit_min(false); } });
	    lim_max_v = getDrawer(0,3).addModel("Button-S1-P7", "l max").setSwitch().setSwitchState(val.limited_max)
			.addEventSwitchOn(new nRunnable() { public void run() { 
    			if(event_paused) val.pauseEvent();
				val.set_limit_max(true); } }).setInfo("limited max")
			.addEventSwitchOff(new nRunnable() { public void run() { 
    			if(event_paused) val.pauseEvent();
				val.set_limit_max(false); } });

	    getDrawer(0,3).addModel("Button-S1-P4", "do_event").setSwitch().setSwitchState(val.doevent())
			.addEventSwitchOn(new nRunnable() { public void run() { 
				val.doEvent(true); } }).setInfo("do_event")
			.addEventSwitchOff(new nRunnable() { public void run() { 
				val.doEvent(false); } });
	    getDrawer(0,3).addModel("Button-S1-P5", "pause_event").setSwitch().setSwitchState(event_paused)
			.addEventSwitchOn(new nRunnable() { public void run() { 
				event_paused = true; } }).setInfo("pause_event")
			.addEventSwitchOff(new nRunnable() { public void run() { 
				event_paused = false; } });

	    do_incr_quatro(2, 1, 10, 100, 1000);
	    do_incr_quatro(4, -1, -10, -100, -1000);

	    do_mult_quatro(6, 1.01F, 1.1F, 2, 10);
	    do_div_quatro(8, 1.01F, 1.1F, 2, 10);
	  ...
	  private void do_incr_quatro(int p, float m1, float m2, float m3, float m4) {
			incr_mod(p, 1, m1);
			incr_mod(p, 2, m2);
			incr_mod(p, 3, m3);
			incr_mod(p, 4, m4);
	  }
	  private void do_mult_quatro(int p, float m1, float m2, float m3, float m4) {
		  mult_mod(p, 1, m1);
			mult_mod(p, 2, m2);
			mult_mod(p, 3, m3);
			mult_mod(p, 4, m4);
	  }
	  private void do_div_quatro(int p, float m1, float m2, float m3, float m4) {
		  div_mod(p, 1, m1);
			div_mod(p, 2, m2);
			div_mod(p, 3, m3);
			div_mod(p, 4, m4);
	  }
	  private void incr_mod(int p, int n, float m) {
		  String pre = "+";
		  if (m < 0) pre = "-";
		  getDrawer(0,4).addCtrlModel("Button-N"+n+"-P"+p, pre)
		  	.setRunnable(new nRunnable() { public void run() { 
		  		if(event_paused) val.pauseEvent();
		      if (ival != null) ival.add((int)m);
		      if (fval != null) fval.add(m);
		      up_limit_view(); } }).setInfo("add "+m);
	  }
	  private void mult_mod(int p, int n, float m) {
		  getDrawer(0,4).addCtrlModel("Button-N"+n+"-P"+p, "x")
		  	.setRunnable(new nRunnable() { public void run() { 
		  		if(event_paused) val.pauseEvent();
		      if (ival != null) ival.mult((int)m);
		      if (fval != null) fval.mult(m);
		      up_limit_view(); } }).setInfo("mult "+m);
	  }
	  private void div_mod(int p, int n, float m) {
		  getDrawer(0,4).addCtrlModel("Button-N"+n+"-P"+p, "/")
		  	.setRunnable(new nRunnable() { public void run() { 
		  		if(event_paused) val.pauseEvent();
		      if (ival != null) ival.div(m);
		      if (fval != null) fval.div(m);
		      up_limit_view(); } }).setInfo("div by "+m);
	  }
